...
- [vllm](file:///home/michael/code/local-agents/vllm) — builds `vllm-qwen36` package for vLLM inference server.
...
- `/vllm` — Folder for the `vllm-qwen36` package
...
### vLLM Qwen 3.6
Navigate to `vllm/` and run `make install`. Then, enable/start the service:
```bash
